...
`agentic-blueprints-skills` is a self-contained skills plugin for cross-tool coding agents. It enforces AMM-style discipline (assess / design / implement / review) for enterprise agents.

## Entrypoint

Start at `skills/using-agentic-blueprints/SKILL.md`. It is the gateway; it routes to one of four sibling skills based on intent.
...
- `references/patterns.md` — pattern families per AMM level (functional signature + controls activated + test asserts for L3–L10; `no v0 pattern family` for L1, L2).
- `references/rosetta.md` — naming-agnostic recognition guide for every control and every pattern family.
