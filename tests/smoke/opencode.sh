...
[[ -f .opencode/plugins/agentic-blueprints-skills.js ]] || { echo "opencode.sh: plugin JS missing"; fail=1; }
if [[ $fail -ne 0 ]]; then exit $fail; fi

node --check .opencode/plugins/agentic-blueprints-skills.js
...
const mod = await import(path.resolve(repoRoot, ".opencode/plugins/agentic-blueprints-skills.js"));
const plugin = mod.AgenticBlueprintsSkillsPlugin || mod.default;
