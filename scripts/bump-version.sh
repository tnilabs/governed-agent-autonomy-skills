...
set_marketplace_plugin_version() {
  ...
  [[ -f "$f" ]] || { echo "missing: $f"; return 1; }
  local tmp; tmp=$(mktemp)
  jq --arg v "$new" '.plugins |= map(if .name == "gaam" then .version = $v else . end)' "$f" > "$tmp"
  mv "$tmp" "$f"
  echo "  updated $f (plugins[].version where name==gaam)"
}
...
versions=$(
  ...
  jq -r '.plugins[] | select(.name=="gaam") | .version' .claude-plugin/marketplace.json
)
