...
2. Treat level descriptions, requirements, controls, and record/schema names as GAAM semantic anchors. Search for semantic equivalents, not literal names: same capability, evidence, failure prevented, and runtime boundary under local vocabulary, services, workflows, or external systems.
...
- Canonical control names, pattern IDs, and record/schema names are trace anchors and examples. Do not require user artifacts to use GAAM strings such as approval record, credential lease, audit record, or evidence pack.
